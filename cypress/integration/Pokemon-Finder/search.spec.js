describe("Pokemon Finder - Search", () => {
    /* eslint-disable */
    before(() => {
        cy.log("Comienzo la suite de test");
        cy.fixture('search.json').as('pokemon');
    });
    ...
    beforeEach(() => {
        ...
        cy.request('GET',`${api_url}/search?search=${pokemon}`).as('RequestPokemon');
    });
    ...
    it('Obtener datos del Pokemon', ()=>{
        ...
        cy.request('GET',`${api_url}/search?search=${pokemon}`).should((response) => {
